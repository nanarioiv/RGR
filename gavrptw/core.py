...
def print_route(route, merge=False):
    ...
    for sub_route in route:
        ...
        if not merge:
            print(f'  Маршрут {sub_route_count} водителя: {sub_route_str}')
        ...
        for customer_id in sub_route:
            ...
            time_cost = wait_cost * max(instance[f'customer_{customer_id}']['ready_time'] - \
                                        arrival_time, 0) + delay_cost * max(arrival_time - \
                                                                            instance[f'customer_{customer_id}'][
                                                                                'due_time'], 0)
...
def run_gavrptw(instance_name, unit_cost, init_cost, wait_cost, delay_cost, ind_size, pop_size, \
    ...
    for gen in range(n_gen):
        ...
        for ind, fit in zip(invalid_ind, fitnesses):
            ind.fitness.values = fit
        print(f'  В поколении {len(invalid_ind)} особей')
        ...
        print(f'  Минимум {min(fits)}')
        print(f'  Максимум {max(fits)}')
        print(f'  Среднее {mean}')
        print(f'  Дисперсия {std}')
    ...
    print(f'Лучшая особь: {best_ind}')
    print(f'Фитнесс-функция: {best_ind.fitness.values[0]}')
    print_route(ind2route(best_ind, instance))
    print(f'Общая сумма: {1 / best_ind.fitness.values[0]}')
    if export_csv:
        ...
        print(f'Запись в файл: {csv_file}')
